...
### 6. PostgreSQL for All Persistence

A single PostgreSQL instance handles events, decisions, audit, and configuration.
...
- See [ADR-002](./decisions/ADR-002-POSTGRESQL.md) for detailed justification
...
- See [ADR-004](./decisions/ADR-004-DETERMINISTIC-RESPONSES.md)
...
Client (Betslip UI)
        ...
        ▼
   ┌─────────┐
   │ API      │  POST /v1/events, /v1/decision, /v1/outcomes
   │ Layer    │  Validation, rate limiting, idempotency
   └────┬────┘
        │
        ▼
   ┌──────────────┐
